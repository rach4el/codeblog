...
def contact_us(request):
    ...
    # This gets the current logged
    # in users submission and
    # displays them by passing them
    # to the template
...
def edit_submission(request, submission_id):
    submission = get_object_or_404(ContactUs,
                                   id=submission_id, author=request.user)
    if request.method == 'POST':
...
def confirm_delete(request, submission_id):
    ...
    return render(request,
                  'contact/confirm_delete.html', {'submission': submission})